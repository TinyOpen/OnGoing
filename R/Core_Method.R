...
OverlapClust <- function(data.simi = data.simi,
                      ...
                      n.comp = 50, alpha = 1)
{
  ...
  return(list(data = data, U.score = U.score, 
...
testlmc<-function(data,r){  # data is a data.frame with two columns with dichotomous or polytomous, rho is the tetrachoric or polychoric correlation coefficient
  n<-dim(data)[1]
  P<-list()
  for (i in 1:2){
    dat<-data[,i]
  ...
  for (i in 1:K1){
    # j=1
    f1<-function(x){dnorm(x)*pnorm((q2u[1]-r*x)/sqrt(1-r^2))}
    If1<-integrate(f1,lower=Low[i],upper=Up[i])$value
    cv<-cv+If1*e1[i]*e2[1]
    cvs<-cvs+If1*e1[i]^2*e2[1]^2
    # j=K2
    fk2<-function(x){dnorm(x)*pnorm((r*x-q2l[K2])/sqrt(1-r^2))}
    Ifk2<-integrate(fk2,lower=Low[i],upper=Up[i])$value
    cv<-cv+Ifk2*e1[i]*e2[K2]
    cvs<-cvs+Ifk2*e1[i]^2*e2[K2]^2
    if(K2>2){
      for (j in 2:(K2-1)){
        f<-function(x){dnorm(x)*(pnorm((q2u[j]-r*x)/sqrt(1-r^2))-pnorm((q2l[j]-r*x)/sqrt(1-r^2)))}
  ...
  #print(i)
  #return(list(r=r,covU=covU,corU=cor(U21,U22)))
  return(list(stat=sstat, pval=pval))
}

# function to determine key features of a cluster-----------------
keyfeat<-function(cdata,criter){   # cluster data
  n<-dim(cdata)[1]
  m<-dim(cdata)[2]
  cmax<-max(apply(cdata,2,max))
  if(cmax==1) {r<-tetrachoric(cdata)$rho}
  else {r<-polychoric(cdata)$rho}
  N<-m*(m-1)/2
  #criter<-0.05/N # Bonfouroni rule for multiple tests
  #criter<-0.01
  Stats<-matrix(0,m,m)
  Pval<-matrix(1,m,m)
  for (i in 1:(m-1)){
  ...
  if(flenp==0){kfeatp<-0}
    ...
    for (ii in 1:flenp){
      ...
      kfeatp[ii]<-paste(colnames(cdata)[fij],colnames(cdata)[fii+1],sep=',')
    }
  }
  Sn<-Stats
  Sn[Pval>criter]<-0
  ...
  if(flenn==0){kfeatn<-0}
    ...
    for (ii in 1:flenn){
      ...
      kfeatn[ii]<-paste(colnames(cdata)[fij],colnames(cdata)[fii+1],sep=',')
    }
  }
  stat<-Stats
  ...
  return(list(kfp=kfeatp, kfn=kfeatn, stat=stat, scrit0=scrit0, scrit1=scrit1))
}
